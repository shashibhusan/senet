...
%>
  ...
  <td class="tablecontent1"><%=rsAdminResult.getString("branch1")%></td>
   <td class="tablecontent1"><%=rsAdminResult.getString("branch2")%></td>
   	<td class="tablecontent1"><%=((rsAdminResult.getInt("branch1"))+(rsAdminResult.getInt("msmedi"))+(rsAdminResult.getInt("branch2")))%></td>
...
<table width="100%" border="0" align="ceneter">
 ...
 <% } else if(divFlag==1){
  String strSql = " SELECT sum(msmedi) as cum,sum(branch1) as cum1,sum(branch2) as cum2,sum(branch3),sum(branch4),sum(branch5),sum(branch6),name FROM tbl_revenue_branch  where user_date >= '"+tYears+"-01-01' and user_date <='"+tYears+"-"+months+"-30' AND   instid='"+instId+"' group by name  ";
rsAdminResult=stmt.executeQuery(strSql);
%>
 <% for(int icnt=1;rsAdminResult.next();icnt++)
{
%>
  <tr>
   <td class="tablecontent1"><%=(rsAdminResult.getInt("cum")+rsAdminResult.getInt("cum1")+rsAdminResult.getInt("cum2"))%></td>
