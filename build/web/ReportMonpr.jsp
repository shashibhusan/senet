...
ResultSet rsCumulative=stmt.executeQuery(strCumulative);
		  ...
		  cum_mon3=rsCumulative.getInt("sum(edp2)");
		   ...
		   System.out.println(""+cum_mon1);
		    System.out.println(""+cum_mon2);
			 System.out.println(""+cum_mon3);
			  System.out.println(""+cum_mon4);
			   System.out.println(""+cum_mon5);
...
rsAdminResult=stmt.executeQuery(strSql);
 ...
 strCumulative=" SELECT sum(vdp_conducted),sum(unit_participated),sum(Amount_Disbursed),sum(SVDPvdp_conducted),sum(SVDPunit_participated),sum(SVDPAmount_Disbursed) FROM tbl_vendor where months >=1 AND months <="+months+" and year='"+years+"' and instid='"+instId+"'";
 ...
 strQuery = "SELECT * FROM tbl_pms_target WHERE year='"+years+"' AND   Inst_Id='"+instId+"'"; 
...
rsAdminResult=stmt.executeQuery(strSql);
 ...
 strCumulative=" SELECT sum(No_Of_unit_benefitted),sum(No_jobs_underTaken),sum(No_Trainees_Trained),sum(No_jobs_Completed),sum(Capacity_Assessment),sum(No_units_Registered)  FROM tbl_workshop where months >=1 AND months <="+months+" and year='"+years+"' and instid='"+instId+"'";
 ...
 rsCumulative=stmt.executeQuery(strCumulative);
	      ...
	      while(rsCumulative.next())
	      {
	       cum_mon1=rsCumulative.getInt("sum(No_Of_unit_benefitted)");
	       cum_mon2=rsCumulative.getInt("sum(No_jobs_underTaken)");
		   
		  cum_mon3=rsCumulative.getInt("sum(No_Trainees_Trained)");
	       cum_mon4=rsCumulative.getInt("sum(No_jobs_Completed)");
		   
		   cum_mon5=rsCumulative.getInt("sum(No_units_Registered)");
	       cum_mon6=rsCumulative.getInt("sum(Capacity_Assessment)");
...
<DIV align="center" class="green"><STRONG> Workshop Details </STRONG></DIV>
